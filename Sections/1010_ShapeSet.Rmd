...
} where $\abs{D_1} = (2+\pi)/2$.

The second dataset is simulated from a distribution with density \eeq[Simulation-2]{
  f_2(\xb; k) = \dfrac{\expo{-\frac{\sqrt{x_1^2 + x_2^2} + \b\frac{x_2^2}{\sqrt{x_1^2 + x_2^2}}}{\s}}}{\s^2\abs{D_2}\fnc{\log}{4}\paren{1 + \expo{-\frac{\sqrt{x_1^2 + x_2^2} + \b\frac{x_2^2}{\sqrt{x_1^2 + x_2^2}}}{\s}}}^2},
} where $\b = 4$ is the parameter for the sine-squared star body \citep{Liebscher2016} and $\s=1$ is the scaling parameter. The corresponding gauge function and logistic-type density generator are, respectively, \eeq[simulation-2]{
...
} where $\abs{D_2} = \pi(2 + \b)/(2(1 + \b)^{3/2})$.
