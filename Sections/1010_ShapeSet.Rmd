...
We denote realizations of the random vectors by the sample $\XC = \xb_1, \dots, \xb_n$. In the bivariate setting, the components of each random vector in Cartesian and polar coordinates are, respectively, \eqq{
...
The techniques for estimating $D$ are motivated by the equivalent problem of estimating $\bdry{D}$. From \mbox{Equation \eqref{Eq:D-boundary}} and Equation \eqref{Eq:boundary}, the boundary of the shape set is \eeq[boundary-u]{
