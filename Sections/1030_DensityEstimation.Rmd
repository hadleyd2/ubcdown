...
In this section, we combine the shape set estimators of Section \@ref(S:shape-set) with estimators of the density generator to develop approaches for estimating a \ac{pdf} $f \in \HC{D}$. The density generator can be estimated parametrically by assuming a parametric form for modelling light-tailed behavior, such as the normal density generator, heavy-tailed behavior, such as the $t$ density generator, or both, such as the Weibull. Since the star-generalized radius variable is a transformation of a random vector from $\R^p$ to $R$, the density generator can also be estimated nonparametrically for any $p$ without encountering the curse of dimensionality.

The iterative approach used to estimate a density $f \in \HC{D}$ on $\R^2$ given known location parameter $\mub = \zerob$ is as follows:

  - Use Kamiya's estimator in Equation \eqref{Eq:K-standard} to produce the first estimate of the shape set, $\est{D}^{(1)}$, and its corresponding gauge function, $\fncD[\est{D}^{(1)}]{n}{\xb}$;
  - The first estimates of the star-generalized radii are attained through the gauge function for the estimated shape set, \eqq{
  \est{r}_i^{(1)} = \fncD[\est{D}^{(1)}]{n}{\xb};
  }
  - By assuming a Weibull-type density generator, \eqq{
    \fnc{f_0}{r} \ \propto \ \expo{-\paren{\frac{r}{\s}}^{\xi}}, \qquad r > 0,
    } with scale parameter $\s > 0$ and shape parameter $\xi > 0$, we are assuming that the data follow a distribution with density \eqq{
    \fnc{f}{\xb} = \fnc{f_0}{\fncD{n}{\xb}} = \frac{1}{\abs{D}\s^2\fnc{\Gamma}{\frac{2+\xi}{\xi}}}\expo{-\paren{\frac{\fncD{n}{\xb}}{\s}}^{\xi}}, \qquad \xb \in \R^2,
    } with shape set $D$. The density for the star-generalized radius variable follows from Equation \eqref{Eq:scale-r}, \eqq{
    \fnc{f_R}{r} = \frac{2}{\s^2\fnc{\Gamma}{\frac{2+\xi}{\xi}}}r\expo{-\paren{\frac{r}{\s}}^{\xi}}, \qquad r > 0.
    } Thus, we can estimate the parameters of $f_0$ using \ac{ml} by maximizing the log-likelihood function for the star-generalized radii. The log-likelihood function is \eqq{
    \fnc{\ell}{\s, \xi; r_1, \dots, r_n} = n\fnc{\log}{2} - 2n\fnc{\log}{\s} - n\fnc{\log}{\fnc{\Gamma}{\frac{2+\xi}{\xi}}} + \sumto{i=1}{n}\fnc{\log}{r_i} - \sumto{i=1}{n}\paren{\frac{r_i}{\s}}^{\xi}.
  }
  - We can now use the \ac{ml} estimates $\paren{\est{\s}, \est{\xi}}$ to specify the density generator and update our estimate of the shape set using free-knot splines with estimator Equation \eqref{Eq:free-knot-estimator} to produce $\est{D}^{(2)}$.
...
Using the simulated dataset from Section \@ref(S:shape-set) with density given by Equation \eqref{Eq:Simulation-1}, we apply the iterative method to estimate $f$. Under the true density generator, the density for the star-generalized radius variable is \eqq{
  \fnc{f_{R1}}{r} = r\expo{-\frac{r^2}{2}},
} and so the distribution function is $\fnc{F_{R1}}{r} = 1-\expo{-r^2/2}$.
...
fullmle.1 <- nlm(nllk.wbl.r, p=c(strt.1$scale, strt.1$shape), r=rstar.1)
...
```{r plots-sim-1, fig.height=7, fig.width=8, fig.cap="\\label{Fig:diag-sim-1}Comparing estimates to true values for Simulation 1", cache=TRUE, echo=FALSE}
bdry.1 <- readRDS(gsub("ubcdown", "", here::here("Thesis", "data", "bdry.1.norm.RDS")))
par(mfrow=c(2,2))
## Kamiya's Shape Set Estimate
plot(bdry.1$x1, bdry.1$x2, type='l', main="Kamiya's Shape Set",
...
plot(bdry.1$x1, bdry.1$x2, type='l', main="Free-Knot Spline Estimate",
     xlim=c(-1.1, 1.1), ylim=c(-1.1, 1.1), xlab=expression(x[1]), ylab=expression(x[2]),
     lty=1, col=scales::alpha("black", 0.75))
...
rm(xi.tmp)
fullmle.1v2 <- nlm(nllk.wbl.r, p=c(strt.1$scale, strt.1$shape), r=rstar.1v2)
