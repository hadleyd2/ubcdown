...
In this section, we develop an iterative approach for estimating a \ac{pdf} $f \in \HC{D}$ on $\R^2$ from data $\xb_1, \dots, \xb_n$ by combining shape set estimators in Equations \eqref{Eq:estimator-Kamiya} and \eqref{Eq:free-knot-estimator} with \ac{ml} estimation for the density generator. Given known location parameter $\mub = \zerob$, the iterative approach is as follows:
  ...
  - By assuming a Weibull-type density generator $\fnc{f_0}{r} \ \propto \ \expo{-\paren{\frac{r}{\s}}^{\xi}}$ for $r > 0$ with scale parameter $\s > 0$ and shape parameter $\xi > 0$, we are assuming the data follow a distribution with density \eqq{
  ...
  - Use the \ac{ml} estimates $\paren{\est{\s}^{(1)}, \est{\xi}^{(1)}}$ to specify the density generator and update the shape set estimate using the free-knot spline estimator in Equation \eqref{Eq:free-knot-estimator} to produce $\est{D}^{(2)}$ and its corresponding gauge function, $\fncD[\est{D}^{(2)}]{n}{\xb}$.
...
Using the simulated dataset from Section \@ref(S:shape-set) with density given by Equation \eqref{Eq:Simulation-1}, we apply the iterative method outlined above to estimate $f$. Assuming a Weibull-type density generator, the true parameters are $\s_0 = \sqrt{2}$ and $\xi_0 = 2$. Results for each step are given in Figure \ref{Fig:iterative-1}. Plot (a) shows the shape set estimate $\est{D}^{(1)}$ against the true shape set. Plot (b) presents a QQ-plot of the star-generalized radius variables for the estimated Weibull-type density generator with estimated scale parameter, $\est{\s}^{(1)} = `r round(mle.1$scale, 2)`$, with standard error $\fnc{\text{SE}}{\est{\s}^{(1)}} = `r round(se.1$scale.se, 2)`$ and estimated shape parameter, $\est{\xi}^{(1)} = `r round(mle.1$shape, 2)`$, with standard error $\fnc{\text{SE}}{\est{\xi}^{(1)}} = `r round(se.1$shape.se, 2)`$ against the true star-generalized radii. Plot (c) presents the updated shape set estimate $\est{D}^{(2)}$ from the free-knot spline estimator. Plot (d) is analogous to Plot (b), but uses the estimated scale parameter, $\est{\s}^{(2)} = `r round(mle.1v2$scale, 2)`$, with standard error $\fnc{\text{SE}}{\est{\s}^{(1)}} = `r round(se.1v2$scale.se, 2)`$ and estimated shape parameter, $\est{\xi}^{(2)} = `r round(mle.1v2$shape, 2)`$, with standard error $\fnc{\text{SE}}{\est{\xi}^{(1)}} = `r round(se.1v2$shape.se, 2)`$. These preliminary results are promising in that the shape set estimates and the density generator parameter estimates better fit the data in the subsequent iteration.
