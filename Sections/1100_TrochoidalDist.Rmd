...
Constraining the parameters from Equations \eqref{Eq:hypo1} or Equations \eqref{Eq:epi1} is difficult since it is not always obvious how changes to $R$, $r$, and $d$ affect the resulting curve. We adopt the parameterization of \citet{Ferreol2017} since it is intuitive and interpretable. The parameters, interpretations, constraints, and domains are given below:
  ...
  - $q \in \set{2, 3, 4, \dots}$ for hypotrochoids and $q \in \set{1, 2, 3, \dots}$ for epitrochoids determines the number of rotations the rolling circle completes as it makes one revolution around the fixed circle. The radius of the rolling circle is thus contrained to evenly divide the radius of the fixed circle by $r = R/q$. When point $P$ lies on the rolling circle, then $P$ touches the fixed circle $q$ times forming $q$ cusps. We use the term cusps generically when referring to $q$. For ease of notation, let $q\in\set{1_e, 2_e, 2_h, 3_e, 3_h, \dots}$ where subscript $e$ refers to epitrochoid with $q$ cusps and $h$ refers to hypotrochoids. Mathematical operations on $q$ ignore the subscript and we do not use subscripts when the type of trochoid is obvious. 
  - $k \in [0, 1)$ is the distance to point $P$ from the center of the rolling circle expressed as a proportion of the rolling circle's radius. Parameter $k$ is related to the distance parameter through $d = kr$. Constraining $k$ in this way ensures that we have a curtate trochoid. Parameter $k$ determines how sharp the curve turns at a cusp. As $k\to0^{+}$, the trochoids converge to a circle and as $k\to1^{-}$, the trochoids converge to cycloids and are not differentiable at the cusps.
...
Restricting the rotation parameter to $\a \in [0, 2\pi/q)$ preserves identifiability. For example, an ellipse can be fully rotated through angle $0 \leq \a < \pi$. Any angle of rotation outside this interval can be generated by applying a rotation from inside the interval. This same restriction applies to other hypotrochoids and to epitrochoids.
...
The trochoidal distribution families are indexed by $q$ and have parameters $k$ and $\a$. All families $q \in \set{1_e, 2_e, \dots, 2_h, 3_h, \dots}$ form a subclass of $\HC{D}$ characterized by having a gauge function describing a curtate trochoid. Using the parametric equations for a hypotrochoid and an epitrochoid, respectively, we first derive the polar functions for the trochoidal boundaries of shape sets. Using the parametric equations for hypotrochoids in Equation \eqref{Eq:hypo-angle}, the polar function is \eqq{
  \fncD[D_h]{r}{\t; q, k, \a} = \frac{1}{q + k - 1}\sqrt{\paren{q-1}^2 + k^2 + 2k\paren{q-1}\fnc{\cos}{qt_{\t}}},
} where $D_h$ denotes that the shape set $D$ has a hypotrochoidal boundary and $t_{\t}$ is the argument $t$ that produces a point with a polar angle of $\t$. Similarly, denoting a shape set $D$ with an epitrochoidal boundary as $D_e$, the parametric equations in Equation \eqref{Eq:epi-angle} give us the polar function for an epitrochoidal boundary, \eqq{
...
} where $\t_{\xb}$ is the polar angle of vector $\xb$. For a trochoidal family $q$ with valid parameter $k$ and $\a$ and a density generator $f_0$ meeting the condition in Equation \eqref{Eq:normalizing-constant}, the densities \eqq{
...
} The area of the region bounded by a curtate hypotrhcoid does not have an analytical form and is by numerically integration of the polar function.
