...
The stochastic representation of \citet[Proposition 1]{Hou2017} is \eeq[Hou]{
  \Xb = \mub + S\Ub,
...
In the bivariate setting, we can simulate from $f \in \HC{D}$ using the simulation over the shape set by following Algorithm \ref{Alg:sim-shapeset}. The efficiency of this algorithm is affected by the ratio of the area of the shape set to the area of the unit circle.
...
\begin{algorithm}[H] \label{Alg:sim-shapeset}
  ...
  Find the maximum distance from $\mub$ to $\pd D$, $\rho^{(\text{mx})} = \maxa[\t \in \S^1]{\fncD{r}{\t}}$, with $r\!_D$ in Equation \eqref{Eq:D-boundary}\;
  $i = 1$\;
  \For{$i = 1, \dots, n$}{
