...
} for shape set $D$ with gauge function $\fncD{n}{\cdot}$ and center $\mub\in\R^p$. Then for a density $f \in \HC{D}$, \eqq{
  \fnc{f}{\xb} = \fnc{f_0}{\fncD{n}{\xb - \mub}; \gbm} = \fnc{f_0}{r; \gbm}.
} Since the value of $R$ depends on $\Xb$, we denote an observed star-generalized radius as $r_{\xb}$ or $r_i$ corresponding to observation $\xb$ or $\xb_i$, respectively.
