...
Free-knot splines have been fit to data using \ac{rjmcmc} by \citet{Denison1998} and \mbox{\citet{DiMatteo2001}}. By combining free-knot splines with hyperspherical log-splines, \citet{Ferreira2005} construct a highly flexible framework for modelling the shape of density level sets for densities \mbox{in $\HC{D}$}. Unfortunately, specification of the density generator is required a priori, and \citet{Ferreira2005} assumes various density generators and selects among them using Bayes factors \citep{Kass1995}. In addition, estimation of \mbox{estimating $\mub$} is done through random-walk Metropolis-Hastings moves to sample each component of $\mub$ independently of the free-knot spline. 

We now provide an outline for the estimation approach of \citet{Ferreira2005} for the shape of density contours in the bivariate setting under the assumption $\mub = \zerob$ is known. Given data $\xb_i\in\R^2$ for $i=1, \dots, n$, let the polar coordinates be $\wb_i = \paren{\t_i, \rho_i}$ with respect to known $\mub$. We model the log-transform of the conditional expectation of the radial variable in \mbox{Equation \eqref{Eq:log-spline}}.
...
\end{subfigure}
\caption{Free-knot spline estimate (in blue) of shape set $D_1$ (in grey) having gauge function in Equation \eqref{Eq:simulation-1} and 95\% credible intervals (in red) for the dataset simulated according to density $f_1$ in Equation \eqref{Eq:Simulation-1} when assuming a density generator of normal-type (left plot), $f_0 \propto e^{-r^2/2\s^2}$, and of logistic-type (right plot), $f_0 \propto e^{-r/\s}/(1 + e^{-r/\s})^2$; the true density generator is of normal-type}
...
We estimate $D_2$ similarly. Here, the misspecified generator is the normal-type density generator given by $f_{01}$ in \mbox{Equation \eqref{Eq:simulation-1}}, and the correct density generator is the logistic-type given \mbox{by $f_{02}$} in \mbox{Equation \eqref{Eq:simulation-2}}. The estimates under misspecification and correct specification are presented in the left and right plots of \mbox{Figure \ref{Fig:fks-plots-2}}, respectively. For this dataset, the choice of density generator does not have a significant effect, suggesting that density generator misspecification may not always be a primary concern. The conditions under which misspecification may not be a problem are yet to be discovered.
...
\end{subfigure}
\caption{Free-knot spline estimate (in blue) of shape set $D_2$ (in grey) having gauge function in Equation \eqref{Eq:simulation-2} and 95\% credible intervals (in red) for the dataset simulated according to density $f_2$ in Equation \eqref{Eq:Simulation-2} when assuming a density generator of normal-type (left plot), $f_0 \propto e^{-r^2/2\s^2}$, and of logistic-type (right plot), $f_0 \propto e^{-r/\s}/(1 + e^{-r/\s})^2$; the true density generator is of logistic-type}
